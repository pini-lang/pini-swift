...
| 测试函数块 `|test`（`名称|test(参数元组,) -> (返回元组,)` / `{名称|test}(签名)`；`pini test <file>` 收集顶级 `|test` 逐一执行；参数按类型注入零值；`Interpreter.runTests` 为运行时入口） | 已定义（v0.42.0，见 G41） | ✅ | Provisional | `pini test` 子命令 / `TestBlockTests` / `TestBlockSwiftTests` / `examples/test.pini` |
...
| 扩展块 `((T))`/`{{T}}`/`[[T]]`/`<<T>>`（规则 3.2/3.14：类型体只含字段/用例，方法须移扩展块并显式 `\|self`/`\|Self`；扩展块内禁自由函数；trait 扩展允许抽象签名） | 已定义且已实现（ADR-016，任务 #12） | ✅ | Provisional | `Parser.parseExtensionDecl` / `ExtensionDecl.swift` / `examples/struct.pini` 等 |
...
| 下标 `a[i]`（`subscript`） | 部分（语法已定义 §A.2.5 postfix-suffix；越界/缺键语义未定） | ✅ | Experimental | `Parser`/`Interpreter` 下标路径 |
| `Self` 关键字 | 已定义（类型层出现 §A.2.6；trait 签名内 `Self` 返回类型经 conformance 校验替换为具体实现类型——`TypeChecker.replaceSelf`，抽象/默认实现均覆盖） | ✅ | Provisional | `Token.swift` `Keyword.Self`、`TypeChecker.replaceSelf` / `TraitConstraintTests.testSelfReturnTypeConformance`/`testSelfReturnTypeInDefaultImplementation` |
...
| `import` / `export`（解析 + 跨文件 enforce） | 已定义（P4 v0.23） | ✅ | Provisional | `Parser.parseImportDecl`、`Package` 跨文件可见性 enforce |
| `module.toml` 模块清单 | 已定义（P4 v0.23） | ✅ | Provisional | `Package`/`FileLoader` 解析 + 跨文件符号 + 可见性 enforce（边界细则见 G15） |
| 标准库函数（18 个） | 部分（G14） | ✅ | Provisional/Experimental | `Interpreter` 内建分发 |
...
- **元数据**：`module.toml` 描述模块名 / 版本 / 依赖（P4 落地）。
...
- **FFI 配置**：`module.toml` 中 `[ffi] abi = "C"` 可省略，默认 C；不引入文件级 / 块级 ABI 覆盖；混合 ABI 拆分模块处理。
...
- **`[名称|foreign]` 块**：声明外部 C 函数签名，块内函数自动视为 `|unsafe`；不接收内联 ABI 参数，继承模块级 `module.toml` 设定的 FFI ABI（默认 `"C"`）。
...
- **`[ffi]` 模块配置（模块级，§6.1）**：`module.toml` 可声明 `[ffi]` 表（前向配置，Phase 2b 起消费）：
...
> **已闭环缺口索引**：正文引用的以下 G 号已随版本闭环（已定义 / 已落地 / 已采纳 / 候选），故不列入上表；对应落点：G8（trait 约束求解，Experimental，§2.4.1）、G9（ARC，见下 Optional 条）、G11（块标签 `scope 块标签:`，§2.4.1/ADR-013）、G13（注释 `;`/`#`，§A.1.4）、G14（文件 IO，§2.4.1）、G15（模块系统边界，§2.5 残留）、G17（数组/字典/集合字面量，§2.4.1）、G18/G24（泛型与运行时单态化，Experimental）、G28（match 子块 + `case _:`，§2.4.3）、G29（匿名函数，§2.4.1）、G30（nil，§2.4.1）、G31（`?T` 糖，§2.4.1）、G32（step，§2.4.1）、G34（COW 值语义，§2.4.1）、G35（`#` 文档注释，§2.4.1/§A.1.4）、G36（for-in，§2.4.1）、G37（扩展块，候选 §A.5.3）、G39（defer 语义，候选 §A.5.5）。
...
> - 具名枚举关联值（`[E] case A(x: T)`）—— 语法已在 §A.2.2 `associated-param` 四形态定义；构造/求值细节按张力 T4 待钉定（示见 `examples/enum.pini`）。
...
> - 内建函数全集（abs / cancel / contains / isCancel / join / joinAll / joinWithin / lower / max / min / readFile / readLine / sleep / split / sqrt / substring / upper / writeFile，共 18 个）—— 较 §3 G14「标准库范围」更具体，签名集未钉住，按 Experimental；其中 `cancel`/`isCancel`/`join`/`joinAll`/`joinWithin` 为并发原语，其**模型**由 G12（Stable）定义，但**函数签名**未钉住（Experimental）。
> - `Optional` —— 类型层糖 `?T`（G31）与 `nil`（G30）已定义（§2.4.1 / §A.2.6）；运行时释放/提升语义未定（与 G9 ARC 关联）。
> - 内建错误类型 `Error()` / `Result` / `CancelError()` —— 由 G12（Stable）纳入异步错误模型；`Error`/`CancelError` 构造与 `Result` 成员语义随 G12 已定义，细节（构造参数、字段）待钉定。
> - 下标 `a[i]`（`subscript`）—— 语法已在 §A.2.5 `postfix-suffix` 定义（容器=任意前序表达式，`(expr)[i]` 天然支持）；越界/缺键运行语义未定（草稿（`a[i] 下标访问` 小节））。
> - `Self` 关键字（类型内自指）—— 类型层出现已定义（§A.2.6 type-annotation `'self'|'Self'`）；作用域运行语义未定。
...
(* 首字符：Unicode 字母或下划线；后续：Unicode 字母/数字/下划线
   '_' 开头的 _名称 走专用访问控制路径 *)
...
KEYWORD      ::= 'var' | 'let' | 'func' | 'enum' | 'object'
               ...
               | 'self' | 'Self' | 'defer' | 'import' | 'export'
               | 'nil' | 'capture' | 'await' | 'wait'
               | 'unsafe' | 'test' | 'foreign' | 'detach'
               | 'scope';
(* 共 33 个。scope 保留但不再使用 *)
...
top-level-decl  ::= import-decl | export-decl
                  | struct-decl | object-decl | enum-decl | func-decl
...
(* 行首 '{' → 函数块或对象块 *)
func-decl       ::= '{' IDENT ['|' 'func'] ['<' generic-params '>'] '}' func-rest;
object-decl     ::= '{' IDENT ['|' 'object'] ['<' generic-params '>'] '}' type-body;
...
method-decl     ::= IDENT ['|' 'self' | '|' 'Self'] func-signature [func-body];
...
modifier        ::= 'func' | 'self' | 'Self' | 'test' | 'unsafe' | 'foreign';
...
defer-stmt      ::= 'defer' control-block;
pass-stmt       ::= 'pass';
...
| 3.2 | **类型体内禁止函数声明** | 类型体内只允许字段声明。解析器在类型体中遇到函数声明头（`IDENT '('` 或 `IDENT '|' modifier`）→ 报错：方法应移至同文件扩展块中，并显式使用 `\|self` 或 `\|Self` | Parser.swift（已实现，ADR-016）：parseStructDecl/parseObjectDeclContent/parseEnumDeclContent 抛 invalidStatement（`name|func` 顶级函数除外）；methodDefaultAssumption 状态机已移除 |
...
| 3.13 | **标签语法消歧** | 语句位置遇到 `IDENT '|'`：若 `'|'` 后是 `'if'`/`'while'`/`'for'` → 解析为带标签语句（`标签\|if` / `标签\|while` / `标签\|for`）；若 `'|'` 后是其他 token → 回退为按位或表达式。声明位置（模块顶层/扩展块内）：`IDENT '|'` 后是 `'self'`/`'Self'`/`'func'`/`'test'`/`'unsafe'`/`'foreign'` → 方法/函数声明修饰符 | Parser.swift（已实现，ADR-014）：parseStatement 标签分派 + parseIf/parseWhile/parseFor(label:)；`scope` 关键字转 reserved-error（G44） |
| 3.14 | **扩展块内禁止自由函数** | 扩展块（`((...))`/`{{...}}`/`[[...]]`）内只允许带 `\|self` 或 `\|Self` 的方法声明。遇到无这些修饰符的函数声明 → 报错：自由函数应移至模块顶层 | Parser.swift（已实现，ADR-016）：parseExtensionDecl（`((T))`/`{{T}}`/`[[T]]`/`<<T>>` 四种扩展，含泛型 `((盒<T>))`）校验 |
| 3.15 | **枚举关联参数仅位置类型** | 枚举用例括号内只接受 `type-annotation {',' type-annotation} [',']`。不允许 `IDENT ':'`（具名）、`IDENT '='`（默认值）或裸字面量。解析到这些形式 → 报错 | Parser.swift（已实现，ADR-016）：parseEnumCase 拒具名/默认/裸字面量；TypeChecker.checkEnumCaseConstruction 拒具名实参（TypeError.enumCaseArgumentLabel，诊断码 E4-014）；匹配/构造/显示全部位置化 |
...
> - 模块顶层：自由函数 → 合法；方法声明（带 `\|self`/`\|Self`）→ 报错（提示移至扩展块）
---
...
`find examples -name "*.pini"` → **45/45 PASS**（含 multifile/package-demo 子目录）。EBNF §2 覆盖全部已实现语法构造。
