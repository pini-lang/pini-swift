...
> **性质**：本文件是一份**故意不完美、版本化、随演进完善的权威规范**。它是代码、README 与示例应当对齐的**单一事实源（single source of truth）**。`Pini草稿.md` 降级为「设计意图与理由（rationale）」附录，README 与示例必须 conform 到本规范。**（2026-09-04）草稿不随本仓分发**（外部文件，见 `.gitignore`）；文中对草稿的段落引用仅作历史溯源标签，其规范性内容均已收编入本规范。
>
...
本规范 spec v0  ──(权威，代码/文档/示例对齐对象)
   │
   ├── Pini草稿.md        ── 设计意图/理由（rationale），不可单独作为兼容性依据（不随本仓分发，见 .gitignore）
...
- **证据表（TOML）**：存于 `evidence-table.toml`（机器可管理/可 diff/可脚本过筛）。每条记录 = `{ assertion / spec_ref | code_ref（符号定位 + 软行号） | validated_at 时间戳 | status }`。状态为 `FRESH`/`STALE`/`PENDING_DELETE`（三态定义见下「自动清扫」）。
...
- **自动清扫（Automatic Sweep，MUST）**：清理义务由脚本履行，**不得**依赖人工或代理的记忆——代理每次会话从零开始，无触发器可提醒。规则：
  - **时效三态**：距 `validated_at` ≤60 分钟 → `FRESH`；>60 分钟且 ≤72 小时 → `STALE`（留待刷新；保留，但不得作为写入依据）；>72 小时 → `PENDING_DELETE`（标成待删）。
  - **两阶段顺序**：每次清扫**先删除**上一轮已标 `PENDING_DELETE` 的条目，**再**重算存活条目的状态。同一轮内**不得**删除本轮刚标记的条目。已刷新（`validated_at` 被更新）的条目即使曾被标记也不删除。
  - **最小间隔 24 小时**：两次清扫间隔不得小于 24 小时（记于 `meta.last_sweep`）。该间隔即宽限期——标记为待删后至少存活 24 小时。
  - **触发点**：`git commit`——经 `hooks/pre-commit` 调用 `tools/evidence_sweep.py --hook`。清扫结果经 `git add` 进入本次提交，故删除前的状态必存于父提交（可恢复）。
  - **被引用条目照删**：清扫**不因**条目被引用而豁免。删除造成的悬空引用记入 `meta.dangling_ids`（引用位置见脚本输出），由人工处置——债务必须留在表里可见，不得只打日志。
  - **容量阈值 100 条（advisory）**：条数 >100 时清扫**仅告警**，不触发任何强制删除。规模由时效规则控制，不设硬性上限。
  - **ID 空间**：`E-NNN`，三位数字。ID 是给人与代理读的标识符，**不是配额**；脚本不分配 ID、不重编号、不复用槽位。
...
- `[...]` 非行首 = 数组 / 字典字面量；`{...}` 非行首 = 集合字面量；`(...)` 非行首 = 元组 / 优先结合；`<>非行首` = 泛型实参。字典字面量为 `[键 = 值, ...]`（批 3：关联记号统一为 `=`，见下方记法原则）。
- **记法原则（批 3，2026-09-02）**：**`=` = 值的注入方向**（实参标签 `f(a = 1)`、字典条目 `["k" = 1]`、元组标签 `(a = 1,)`、默认参数 `n: I32 = 1`）；**`:` = 类型与块的标注方向 + 值的取出方向**（类型标注 `n: I32`、块开启 `if c:`、match 具名绑定 `case A(x: v):`）。旧写法 `f(a: 1)` / `[k: v]` / `(a: 1,)` **已废弃**（宿主报错并给出迁移提示）。切片 `a[i:j]` 的 `:` 是分隔符，不属关联记号，不变。
- **空字典**无字面量形态（`[:]` 为全切片）；用类型构造 `字典<键, 值>()` 取得空字典。
- **消歧锚点**：当前以「是否出现在行首」为唯一规则（已知脆弱，见 §3 G2；本版本固定该法，未来可换算法）。
- **括号内跨行（批 1.5，A12 方案 B / 路 C，2026-09-01 落地）**：最内层未闭合括号为「普通括号」时，括号内 **NEWLINE 等同空白、缩进不参与**（括号深度即结构）：行尾不发 newline 记号、行首不发 INDENT/DEDENT。
- **块携带括号**：开括号后**同行**紧跟块开启关键字 `func`（草稿「原地调用 IIFE」及实参位函数字面量的包裹形态）——其内部布局**完全照常**（newline/INDENT/DEDENT 照常发射，token 流与单行形态同一机制）。四个括号对在**行首**是块声明定界符（§2.1），块头单行配平，不受抑制影响。
- **已裁边界**：`< >` 不参与深度跟踪（泛型实参维持单行；比较运算 `a < b` 与泛型同形，词法层不可消歧）；未闭合括号到 EOF **宽松静默**（ADR-021 契约，语法错误后移至解析层）；不匹配 closer **一律弹栈**（限制级联）；`` `func` `` 反引号转义不构成块携带；块携带判定前瞻**仅限同行**。F5 钉定批（批 2）按「缩进块 + 顶格块 + 括号内记号流」三维度收口。

### 2.3 程序结构
- 顶级 = 单行标识符声明 与 复合块**交替**；块边界由下一个顶级声明隐式关闭（无 `}` 结束符）。
- **缩进敏感**：顶级声明顶格；函数体（含顶级函数）必须缩进 ≥1 层，缩进亦用于标识控制流子块边界。
- **块文法（F5，2026-09-02 钉定）**：§A.2 EBNF 只描述记号序列，不含 INDENT/DEDENT/空行记号，块边界语义由本节规定（顶格块无缩进变化，缩进记号方案无法覆盖，故采「显式声明 + 专节规定」）：
  - **缩进块**（函数体、控制流子块、缩进的声明块体）：INDENT 开启、DEDENT 闭合；**空行与注释行透明**（不闭块）。
  - **顶格块**（四类块头 `(名)` `{名}` `[名]` `<名>` 的体，含扩展块）：体行**可顶格或缩进**（两种写法均合法）；**空行与注释行透明**；闭合于**下一个顶级声明的起点**（块头 / `名称|func` / import-export 块）；体内仅接受该块的合法行（字段块内裸语句 → E2-001）。
  - **括号内记号流**：见 §2.2 括号内跨行（G55）——第三维度，批 1.5 已钉。
  - 依 §1.4 实测钉定：六探针（缩进体 / 顶格·缩进字段块空行透明 / 下一顶级声明闭合 / 裸语句 E2-001 / 扩展块 S9 证据）+ 差分实验（字段块内含空行，宿主与自举 MATCH 253）。
...
| 块标签 `标签|控制流关键字`（标签落在 if/while/for 上；`break 标签`/`continue 标签` 按标签名定向；旧 `scope 块标签:` 已逆转，ADR-014） | 已定义且已实现（ADR-014；`scope` 关键字转 reserved-error） | ✅ | Provisional | 规则 3.13（已实现）；`examples/control-while.pini` |
...
| 下标 `a[i]`（`subscript`） | 已实现（安全模型 G48 三通道：`a[i]` 安全断言→`T`，越界读 **panic**（E5-005）；`a.get(i)` 安全可选→`Optional<T>`，越界 `.none`；`unsafe a.getUnchecked(i)` 不安全→`T`，越界 UB。负索引尾部计数 + 切片语法 `a[i:j]`，见 §A.2.5 / `SubscriptStrategies.swift`） | ✅（解释器端；LLVM 端 `a[i]` 与运行时一致，`.get`/`.getUnchecked` 未实现 → IR 生成报 unsupported） | Experimental | `Parser`/`Interpreter` 下标路径 / `SubscriptStrategies.swift` / `proposal-subscript-safety-channels-2026-09-01` |
...
- **依赖图与跨模块访问（G52）**：
  ...
  - **import 键位的 `_` 语义（D-4，2026-09-02 钉定）**：`_别名 = path` = **注入导入**——目标模块 public 符号进入**本文件**裸名空间（文件级全导入），可**裸调用**或以 `_别名.符号` 限定调用；**非 `_` 别名必须限定调用**（两通道不混）。注入符号与本文件任何可裸引用名（顶级/局部声明、其他注入、显式别名、内建）相撞 → **E3-013 歧义错误**，修复 = 改写显式别名并限定调用（不静默解析）。别名不必等于目标模块名——不一致仅发 **E7-002 弱警告**（检查不强制）。v1 边界：注入不跨模块传递（被引入模块内部的 `_` 导入不上提）；单文件 run 无语义检查（运行时兜底仍有歧义防御）。
  - **`_` 记法语义位置表**：声明位（`_x`/`_lib.pini`）= package 级可见性；export 键位（`_A = 符号`）= 导出为 package 级；import 键位（`_别名 = path`）= 注入全导入（本条）；模式位（`case _:`）= 通配。四义靠位置区分。
...
(* 缩进敏感块文法（F5）：块边界语义已钉定于 §2.3「块文法」（2026-09-02）——本 EBNF
   只描述记号序列，不含 INDENT/DEDENT/空行记号。原维护注中「顶格块由空行闭合」与
   「空行只作分隔」两句自相矛盾，经实测裁决（字段块内含空行，宿主与自举解析器
   MATCH 253；空行在两套 lexer 中均不产记号）：**空行与注释行透明，不闭块**。 *)
...
(* D-1 裁决（2026-08-31）：块头名 = **当前文件名**（去 `.pini` 后缀），解析器校验
   ...
   块形式为本产生式唯一形态；宿主裸语句已裁决移除（G52 批 1，已落地；
   裁决收编见 `docs/spec/issue/archive/issue-module-system-rules-2026-08-28.md` 状态块）。 *)
...
(* G51④/P0（2026-08-31 宿主落地）：defer 双形态——相邻单行语句 + `:` 块形式
   （草稿 §defer 块退出前清理）。块形式的体作为**一个** defer 项入 defer 栈，
   包含块退出时按书写序执行；跨 defer 项的 LIFO 由 defer 栈保证（G39 语义）。
   块体作用域独立（内部绑定不外泄），与 scoped-block 同构。 *)
