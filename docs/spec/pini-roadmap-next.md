...
| **IO 相对路径基准**（批 5） | `docs/spec/issue/proposal-io-path-base-2026-09-02.md`（①②，D-1~D-6 已裁决） | 🔶 **实施中（2026-09-02）**：阶段 2 实现 + 阶段 3 spec/ADR-030/G58/A13 出账/迁移说明 §E/证据 E-087..090 已落地（`01baf63` 等）；剩阶段 4 测试补齐 + 阶段 5 全量/gate/基线 | 采方案 A 三段式（绝对原样 / `./` 运行时 CWD / 其余相对程序基准）；自举从仓根直跑 70/0 已实测 |
