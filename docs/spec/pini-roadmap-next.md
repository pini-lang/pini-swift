...
| **T2** | 模块化深化（依赖解析 + 清单格式） | 用户② | 3 | 3 | 0.7 | 6 | **1.05** | **已交付（G52，2026-09-04 收口）**：模块边界 / 无环依赖图 / `import` 语义 / 清单**双通道**（`require` 模块 / `resources` 资源）+ `tap` + `replace` 已决议落档；清单格式已定（沿用 TOML，不立 `.pnmod`），`[dependencies]` 移除。**宿主实现已由批 1 / 批 6 / 批 7 交付**：双通道解析、经典 MVS、`pini-summary.toml`、SHA-256（TOFU）、`pini mod` 四命令、build 漂移检查、**远程 tap 抓取**（`github:`/`git:`）、resources 落地。**剩余**：`[[bin]]/[lib] entry` 未消费、`graph.order` 未被解释器消费、resources 根检、`[replace]` 两种形态、资源寻址 API —— 均登记于 `docs/spec/issue/archive/issue-module-system-rules-2026-08-28.md` §9 Def-1..Def-8 |
