...
| **T2** | 模块化深化（依赖解析 + 清单格式） | 用户② | 3 | 3 | 0.7 | 6 | **1.05** | **已交付（G52，2026-09-04 收口）**：模块边界 / 无环依赖图 / `import` 语义 / 清单**双通道**（`require` 模块 / `resources` 资源）+ `tap` + `replace` 已决议落档；清单格式已定（沿用 TOML，不立 `.pnmod`），`[dependencies]` 移除。**宿主实现已由批 1 / 批 6 / 批 7 交付**：双通道解析、经典 MVS、`pini-summary.toml`、SHA-256（TOFU）、`pini mod` 四命令、build 漂移检查、**远程 tap 抓取**（`github:`/`git:`）、resources 落地。**剩余：无** —— G52 工单 §9 全部 12 项缺陷已于批 8/9 处置完毕（修复 / 定性更正 / 明确不提供），见该工单状态块五批次表 |
...
- **T2 模块化**：清单格式与四条规则已由 **G52** 定稿（不另立 ADR，随 G52 走；决议见 `docs/spec/issue/archive/issue-module-system-rules-2026-08-28.md`，D1–D28）。**宿主实现已完成**（批 1 语义层 / 批 6 工具链 / 批 7 远程 tap），清单格式选型争议已终结；G52 工单已于 2026-09-04 收口归档，剩余缺陷登记于其 §9。
...
| **批 3**（括号内记法收口） | 实参标签 / 字典条目 / 元组标签 / 枚举具名构造改用 `=`；旧 `:` 记法报错并给迁移提示；match 具名绑定保留 `:` | 形参声明与类型标注；空字典字面量（走类型构造） | 批 2 | 同上 + 迁移后门禁 | ✅ 已落地：`ad10773` / `cbeec25` / `831b3db`（宿主）、`d96c996`（自举）。宿主 1116/0。**随 v0.51.0 发布**（§8.3 治理债全清偿） |
| **批 6**（模块工具链，G52 批 3） | 清单双通道解析（require/resources/tap/replace + `[[ ]]`）；MVS → `pini-summary.toml`；SHA-256 校验和（TOFU）；`pini mod {tidy, refresh, verify, graph}`；build 漂移检查；D-4 隐式别名；旧 `[dependencies]` 报错指引 | 远程 tap 下载（批 7）；资源寻址 API（G52 待定项）；中心化 sumdb | 批 5 | 四项标准出口 + 全链路 tidy→refresh→verify 夹具走通 + 篡改检出 | ✅ **Landed（2026-09-02）**：五阶段全走完（解析 `65539fe` / 核心两件套 `45cd4dc` / 命令与漂移 `97f85bf` / 收尾一笔）。终验：篡改检出 ✓ + 全链路 8 步实测（含跨模块 run「来自 uni」）。**遗留 D-4 隐式别名**（原文语义未定，两种读法，待裁决后另做，不阻塞） |
| **批 7**（远程 tap，G52 批 7） | `TapFetcher`：`github:`/`git:`/`file:` 三协议展开（`<name>` 占位符、D11 缺 org 报错）+ git clone/fetch/checkout + `rev-parse` 取 `commit` + `ls-remote --tags` 列候选；**经典 MVS**（升序候选取满足全部约束的最小版本）；resources 落地 `.pini/resources/<name>/`（消掉静默跳过）；锁文件补齐 `commit` 的读取；批 6 的 D-A 收窄为 `remoteTapNotMaterialized`；**有界不动点迭代**求解（依赖的版本决定其自身的 `[require]`） | `[[bin]]/[lib] entry` 消费；`graph.order` 解释器消费；资源寻址 API；中心化 sumdb | 批 6 | 四项标准出口 + 本地 git 仓库作远端的**离线**全链路 + 传递约束收敛 | ✅ **Landed（2026-09-04）**：`b8b46c3`（阶段 1-2：`TapFetcher` + 经典 MVS）/ `cef9dd4`（阶段 3-4：接线 + resources 落地）。终验：全量 **1157 执行 / 112 跳过 / 0 失败**（批 7 前基线 1144，净增 13）；`git:` 接受本地路径 ⇒ 用 tmp 里的真 git 仓库当远端，离线走通 clone→MVS→checkout→commit→verify→篡改检出；传递依赖 `^1.2` 压过 root 的 `*` 回填（收敛到 1.2.0 而非 2.0.0）；自举仓 `pini check .` 实测通过（15 文件，rc=0）。**收尾补修**：Def-1（resources 根检，双通道双向封闭）与 Def-7（锁文件 `tap`/`source` 回读，verify 报错回显来源），各带 1 个用例。**第二批补修（`df9f91c` / `e65a996`）**：Def-5（三层嵌套用例）、Def-6（`[replace]` 三形态 + `file:` 形态不再往用户本地目录抓取——批 7 回归）。**剩余遗留登记于 G52 工单 §9 Def-2..Def-4 / Def-8..Def-12** |
| **批 9**（G52 全量收口） | `[[bin]]/[lib].entry` 入口定位（声明收窄 `main` 位置，未声明沿用全局找 `main`；`entryMainMismatch` runtime-018）；`graph.order` 定为导出视图 + 三层依赖链钉子测试（Def-2 定性更正）；自举仓 `examples/` 由 R1 自切（Def-4，独立提交）；`versionComponents` 按分量剥 `v`（Def-9，变异验证）；模块扫描双策略按来源划分（Def-11）；锁文件 `tap` 取字典序最小 requirer（Def-12）；资源寻址 v1 不提供（Def-8） | 无（§9 全部闭环） | 批 8 | 四项标准出口 | ✅ **Landed（2026-09-04）**：`9e52cf9`（Def-3）/ `118c73f`（Def-2）等。终验：全量 **1178 执行 / 112 跳过 / 0 失败**（批 8 收口 1167，净增 11：Def-9 的 3 + Def-12 的 1 + Def-11 的 2 + Def-3 的 4 + Def-2 的 1）；entry 四态 CLI 实测（无声明 / 满足 / 指错 rc=1 / src/ 布局）；Def-3 与 Def-12 均经变异测试证明测试非空转 |
| **批 8**（G52 收尾补修） | `[replace]` 三形态补全（版本覆盖 / `file:` 本地 / fork `@版本`），版本类替换并入 MVS 约束；`file:` 替换不再往用户本地目录 `fetch`+`checkout`（批 7 回归）；三层嵌套模块用例（R1 递归排除） | `[[bin]]/[lib] entry` 消费；`graph.order` 解释器消费；D27 后自举仓 `examples/` 无归宿；资源寻址 API；锁文件 `tap` 取边规则；模块边界扫描深度不一致（Def-11）；`versionComponents` 剥 `v` 前缀（Def-9） | 批 7 | 四项标准出口 | ✅ **Landed（2026-09-04）**：`df9f91c`（Def-6，7 用例）/ `e65a996`（Def-5，1 用例）。终验：全量 **1167 执行 / 112 跳过 / 0 失败**（批 7 收尾后基线 1159，净增 8）；fork 替换锁文件 `tap = "replace"`、来源指向 fork；依赖清单的 `[replace]` 实测被忽略（D13 作用域）。**新登记 Def-9..Def-12**（补修过程中发现，未修） |
| **批 5**（G58 IO 路径基准） | 三段式解析（绝对原样 / `./` `../` 运行时 CWD / 其余相对程序基准）；`moduleRoot()`；CLI 六入口注入；LLVM 字面量烘焙 | F6 argv 透传（仅铺路）；LLVM `moduleRoot`（报 unsupported）；跨机可移植（v1 限制）；清单工具（批 6）/ 远程抓取（批 7） | 批 3（v0.51.0） | 四项出口同上（全量 0 失败 / gate 五关 / 基线重标定后重跑 / 两仓提交分离）；**另加终验：自举测试从仓根直跑全绿** | ✅ 已落地：`01baf63`（实现）/ `b6f531f`（spec/ADR-030/G58/A13 出账/迁移 §E/E-087..090）/ 测试一笔（IOTests 6→11）；基线第十六次重标定 `e48449c`。宿主 1121/0；自举从仓根直跑 70/0（终验通过，5 个 CWD 依赖假失败零改动修复） |
...
| 版本 bump（路线图条款：语义变更须 bump） | ✅ 统一 **v0.51.0**（2026-09-02） | ✅ 同左 | ✅ 同左 | 已清偿；两份 CHANGELOG 已记里程碑 |
...
| 项 | 提议载体 | 状态 | 备注 |
|---|---------|------|------|
| **F5 块文法钉定**（缩进块 + 顶格块 + 括号内记号流三维度） | `docs/spec/issue/archive/issue-de-facto-grammar-pinning-2026-08-30.md` §F5 | ✅ **Landed（2026-09-02）**：spec §2.3 块文法 + EBNF 注消解（空行透明裁决）+ 差分探针入语料（MATCH 253） | 提议在册；裁决过程含「空行闭块」历史假设的否定 |
| **IO 相对路径基准**（批 5） | `docs/spec/issue/archive/proposal-io-path-base-2026-09-02.md`（①②，D-1~D-6 已裁决） | ✅ **Landed（2026-09-02，批 5）**：五阶段全走完（§8.1 批 5 行）；ADR-030 / G58 / A13 出账 / 迁移 §E / E-087..090 | 采方案 A 三段式；终验 = 自举从仓根直跑 70/0 |
