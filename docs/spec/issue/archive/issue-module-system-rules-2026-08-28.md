...
- 状态：**Closed / LANDED（2026-09-04，批 9 全量收口）** —— 本文档即决议记录，**R1–R8 / D1–D28 不变**。
  决议、规范登记与宿主实现均已交付，由五个批次承接：
  ...
  | **批 8**（收尾补修一） | 2026-09-04 | `[replace]` 三形态（D13 补完）、三层嵌套用例（Def-5）、resources 根检（Def-1）、锁文件 `tap`/`source` 回读（Def-7）、批 7 的 `file:` 替换抓取回归（Def-10） |
...
>
> **2026-09-04 第三批（批 9）收口**：其余全部处置完毕——**Def-9**（`v` 前缀）、**Def-12**（取边规则）、
> **Def-11**（扫描策略按来源划分）、**Def-4**（`examples/` 由 R1 自切，自举仓独立提交）、
> **Def-3**（`entry` 入口定位 + `entryMainMismatch` 诊断；E3-004 经实测更正：双 `main` 上游已诊断
> 为 `redeclared symbol`，非静默）、**Def-2**（定性更正：解释器递归 `loadImports` 已保证依赖先就绪，
> `graph.order` 定为导出视图 + 三层链测试钉住）、**Def-8**（资源寻址明确 v1 不提供 + 判据）。
> **§9 至此全部闭环**；下表保留为历史记录，删除线行已附处置结果。

| # | 缺陷 | 判据 | 影响 |
|---|---|---|---|
| **Def-1** | **R7 反向根检未实现**：`resources X` 而 X 根含 `pini.toml` 时不报错 | R7 / D20 / D26「双向封闭」 | DoD #5 只兑现一半。把 Pini 模块误配成资源不会被拦，需人工发现 |
| **Def-2** | ~~`graph.order` **只写不读**~~ → **2026-09-04 批 9 处置（定性更正）**：解释器与工具链是两套解析，解释器 `loadImports` 的递归结构已天然保证「依赖先于依赖者就绪」；`graphOrder` 定为**导出视图**（外部工具消费，非解释器输入），并以三层链用例钉住可观测保证 | §3.5 / §6.2 | 接进解释器 = 两套解析合流，属架构改动非缺陷修复；纯注册顺序无可观测副作用，钉的是「依赖环境先于依赖者可用」 |
| **Def-3** | ~~`[[bin]].entry` / `[lib].entry` **全仓零消费点**~~ → **2026-09-04 批 9 已实现**：`entry` 已解析并注入解释器，声明后 `main` 必须落在声明文件（否则 `entryMainMismatch`，runtime-018）；**未声明沿用全局找 `main`**（用户裁决：增量不替换）。**更正**：E3-004 经实测上游已诊断为 `redeclared symbol`，非静默 | §6.2 / G49 | 主模块入口可按文件定位；`[[bin]].name` 仍无消费者（v1 入口符号恒为 `main`，解析处注明） |
| **Def-4** | ~~`examples/` 失去扫描语义归宿~~ → **2026-09-04 批 9 已归位（三选一取方案 1）**：`examples/selfhost/examples/` 自带 `pini.toml`，R1 自切其出编译器源集；`exclude = ["examples"]` 保留、注释改写为 D27 语义。实测：摘掉 exclude 后 `pini check .` 仍 15 文件 ⇒ R1 独立生效；且 exclude 现为可观测 no-op（语料无 `\|test` 块），已写入清单注释 | D27 + R1 | 零行为变更（`pini check .` 15 文件 rc=0、`pini test .` 70 通过，改动前后一致）；自举仓独立提交（two-track） |
...
| **Def-7** | ~~锁文件 `tap` / `source` 字段**只写不读**~~ → **2026-09-04 已补**（`parseSummary` 读取二者，`verify` 报错回显来源） | §3.5 | — |
| **Def-8** | ~~`[resources]` 的**产品内寻址方式**未定义~~ → **2026-09-04 批 9 收口：v1 明确不提供**。判据：当前零真实用例，寻址形式属语言级新增，待第二个用例出现再定型（让抽象浮现）。规范不再留「待定」悬空承诺（project-spec §3.3 注记） | §3.3 | v1 语义止于「落地 + 校验和」；如未来立项寻址，从用例出发设计而非预设形式 |
| **Def-9** | ~~`versionComponents("v1.0")` 返回 `[0]`~~ → **2026-09-04 批 9 已修**：按分量剥 `v`/`V` 前缀（顺带修复 `1.v0` 类畸形输入）；`TapFetcher` / `parseReplace` 的本地剥除**保留**并重新定性为**规范化**（产出用于 ref 名与锁文件 `Constraint.raw`，须为 `1.0.0` 形态）——是分工不是双份真相。回归测试经 stash 回退验证为真（3 用例 6 断言全转红） | §3.2 | `pini.toml` 的 `version = "v1.0"` 不再被当版本 0 |
...
| **Def-11** | ~~扫描深度两处不一致~~ → **2026-09-04 批 9 已修（按用户定性重构）**：两种深度是**既定策略**而非不一致——根级 = 远程清单省解析时间，递归 = 本地目录。原实现划分轴是「主模块 vs 被引入」，改为按**来源**划分：父目录名为 `deps`（D23 落地根）→ 根级，其余递归；两路共用 `FileLoader.parseUnit` 消除漂移源。全量 1171 / 0 失败，止损点未触发 | R1 | 自带 `src/` 布局的模块被 import 时正常加载；demo3 夹具注释同步改写（lex/ 现属 syntax 模块） |
| **Def-12** | ~~取边规则未定~~ → **2026-09-04 批 9 已定**：**字典序最小 requirer 胜出**（`<root>` 前缀 `0x3C` 恒先于模块名 ⇒ 主模块声明优先）。不取「首次落地」：遍历是 LIFO（`popLast`），首次落地 = 最后入队 ≠ 离根最近，且遍历策略一变结果就变。测试夹具经两轮迭代才具区分力（旧规则等价于「最浅 requirer 胜出」，须浅层 requirer 名字典序**更大**才分岔） | §3.5 | 多 requirer 下 `tap`/`source` 可复现；约束与 `imported-by` 仍按边记录，仅 summary 字段去重 |
