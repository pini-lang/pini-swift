...
- 状态：**LANDED（2026-09-04 收口核验）**——本提案 D-1~D-7 已由冲突登记册 **A15**（用户 2026-09-01，D-1~D-7）全量裁决并落地：三通道模型采纳、落点 spec §2.4.1 G48（修订）/ §3 缺口 G56 / **ADR-028**、迁移 `docs/spec/migration-2026-09.md` §A（`]!`→`]` 218 处实测完成）。**D-4（空 unsafe 消耗点行为）由 ADR-028 明确回应**：「`unsafe` 许可是『允许』而非『要求』」，空消耗点无害通过；D-1 批号归属随落地失效（A15 未沿用本表批号框架）。本文件为裁决的需求源头登记，与 A15/ADR-028 互为佐证。
- 提出方：用户
