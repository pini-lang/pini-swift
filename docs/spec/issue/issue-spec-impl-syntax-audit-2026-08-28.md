...
- 状态：已拍板（G51，2026-08-28）；spec 修订完成；宿主收敛项全部落地（最后一项 test 关键字 2026-09-04 批 A 闭环）→ 可归档
...
**宿主收敛待办清单（G51 遗留，按优先级；2026-09-04 实测核验）**：
1. ✅ **已落地**（G52 批 1，2026-08-31）：移除顶级裸 `import`/`export` 语句解析路径，实现 `[名称|import]` / `[名称|export]` 块形式（含 `_` 别名语义——与 `_` 访问控制系统联动，涉及可见性层）。**2026-08-31 已裁决执行**（用户拍板）：理由是本语言顶层未为裸语句留空间（连全局变量都没有），不可能仅为 import/export 破例。为破坏性变更，走 Provisional 破坏窗口；自举 parser / 语料随宿主同步（裁决收编见 G52 归档工单状态块「批 1 裁决收编」节）；
2. ✅ **已落地**（2026-08-31）：`defer` 块形式（`defer:` + 缩进块）；
3. ✅ **已落地**（2026-09-04 批 A）：`test` 收编为关键字（`Keyword.test`，spec 共 34 对齐；`parseIdentifier` 白名单接出 `|test` 修饰符，`Interpreter.runTests` 收集路径不变）；自举 `kw_test` 本就一致，词法双轨消除；
4. ✅ **已出账**：成员内建名劫持——原缺陷不复现，随 ADR-020 步骤 B 派发表驱动消解，扩展覆盖缺口由 H-3 落地修复（裁决与证据见 `docs/spec/issue/archive/issue-conflict-register-2026-08-31.md` §二）；`ImportExportTests` 裸语句用例已改写为块形式（实测 2026-09-04：测试文件中无 Pini 裸语句，唯一 `import` 为 Swift 测试框架自身）。
