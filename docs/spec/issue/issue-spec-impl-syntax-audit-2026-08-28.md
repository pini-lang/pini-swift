...
4. ✅ **已出账**：成员内建名劫持——原缺陷不复现，随 ADR-020 步骤 B 派发表驱动消解，扩展覆盖缺口由 H-3 落地修复（裁决与证据见 `issue-conflict-register-2026-08-31.md` §二）；`ImportExportTests` 裸语句用例已改写为块形式（实测 2026-09-04：测试文件中无 Pini 裸语句，唯一 `import` 为 Swift 测试框架自身）。
