...
final class StdlibTests: XCTestCase {
    ...
    func testStringUpperLower() throws {
        let source = try loadPiniFixture("testStringUpperLower", filePath: #filePath)
    ...
    func testStringContains() throws {
        let source = try loadPiniFixture("testStringContains", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testStringSubstring() throws {
        let source = try loadPiniFixture("testStringSubstring", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testStringSplit() throws {
        let source = try loadPiniFixture("testStringSplit", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testArrayJoin() throws {
        let source = try loadPiniFixture("testArrayJoin", filePath: #filePath)
        let output = try runProgram(source)
        XCTAssertEqual(output, "a-b-c\n", "join 应用分隔符连接数组元素")
    ...
    func testMathAbs() throws {
        let source = try loadPiniFixture("testMathAbs", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testMathMinMax() throws {
        let source = try loadPiniFixture("testMathMinMax", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testMathSqrt() throws {
        let source = try loadPiniFixture("testMathSqrt", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testMathTrig() throws {
        let source = try loadPiniFixture("testMathTrig", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testAbsIntMinDoesNotCrash()  throws {
        // 用运算构造 Int.min（避免大字面量在词法层溢出掩盖 abs 缺陷）
        let source = try loadPiniFixture("testAbsIntMinDoesNotCrash", filePath: #filePath)
    ...
    func testSubstringNegativeIndexClamped() throws {
        let source = try loadPiniFixture("testSubstringNegativeIndexClamped", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testSubstringNegativeStartBeyondEnd() throws {
        let source = try loadPiniFixture("testSubstringNegativeStartBeyondEnd", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testSubstringStartGreaterThanEnd() throws {
        let source = try loadPiniFixture("testSubstringStartGreaterThanEnd", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testSubstringForwardClamp() throws {
        let source = try loadPiniFixture("testSubstringForwardClamp", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testJoinWithNonStringElements() throws {
        let source = try loadPiniFixture("testJoinWithNonStringElements", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testMinMaxMixedTypesThrows()  throws {
        let source = try loadPiniFixture("testMinMaxMixedTypesThrows", filePath: #filePath)
