...
final class TraitDefaultTests: XCTestCase {
    ...
    func testTraitWithDefaultMethodParses() throws {
        let source = try loadPiniFixture("testTraitWithDefaultMethodParses", filePath: #filePath)
        let lexer = Lexer(source: source, fileName: "test.pini")
        let tokens = try lexer.tokenize()
        let parser = Parser(tokens: tokens, fileName: "test.pini")
        let module = try parser.parseModule()
    ...
    func testTraitWithAbstractMethodParses() throws {
        let source = try loadPiniFixture("testTraitWithAbstractMethodParses", filePath: #filePath)
        let lexer = Lexer(source: source, fileName: "test.pini")
        let tokens = try lexer.tokenize()
        let parser = Parser(tokens: tokens, fileName: "test.pini")
        let module = try parser.parseModule()
    ...
    func testStructTraitExtraction() throws {
        let source = try loadPiniFixture("testStructTraitExtraction", filePath: #filePath)
        let lexer = Lexer(source: source, fileName: "test.pini")
        let tokens = try lexer.tokenize()
        let parser = Parser(tokens: tokens, fileName: "test.pini")
        let module = try parser.parseModule()
    ...
    func testDefaultMethodAutoAvailable() throws {
        let source = try loadPiniFixture("testDefaultMethodAutoAvailable", filePath: #filePath)
    ...
    func testBehaviorCoversDefault() throws {
        let source = try loadPiniFixture("testBehaviorCoversDefault", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testTraitMethodWithoutSelfParam() throws {
        let source = try loadPiniFixture("testTraitMethodWithoutSelfParam", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testTraitMethodWithExtraParams() throws {
        let source = try loadPiniFixture("testTraitMethodWithExtraParams", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testSelfAccessibleInDefaultMethod() throws {
        let source = try loadPiniFixture("testSelfAccessibleInDefaultMethod", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testNoTraitImplementationStillWorks() throws {
        let source = try loadPiniFixture("testNoTraitImplementationStillWorks", filePath: #filePath)
        let output = try runProgram(source)
