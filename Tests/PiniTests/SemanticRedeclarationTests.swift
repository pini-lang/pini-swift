...
final class SemanticRedeclarationTests: XCTestCase {
    ...
    func testRedeclaredTopLevelFunction() {
        assertRedeclared(
            """
            foo|func() -> ()
                return
    ...
    func testRedeclaredTopLevelStruct() {
        assertRedeclared(
            """
            (Point)
            x: I32
    ...
    func testDistinctTopLevelNamesAccepted() {
        let source = """
        foo|func() -> ()
            return
        bar|func() -> ()
    ...
    func testRedeclaredLocalVariableInSameScope() {
        assertRedeclared(
            """
            foo|func() -> ()
                var x = 1
    ...
    func testShadowingInNestedScopeAllowed() {
        let source = """
        foo|func() -> ()
            var x = 1
            if true:
    ...
    func testStructFieldMethodConflict() {
        assertRedeclared(
            """
            (Conflict)
            x: I32
    ...
    func testCrossEnumSameCaseNameAllowed() {
        let source = """
        [形状]
        圆(F64,)
        矩形(F64, F64,)
    ...
    func testSameEnumDuplicateCaseNameRejected() {
        assertRedeclared(
            """
            [形状]
            圆(F64,)
