...
final class CPSDifferentialTests: XCTestCase {
    ...
    func testDiffBasicJoin() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffBasicJoin", filePath: #filePath) as String)
    }

    /// 意图：`<=` 出现在 call 实参内部（`print(await child())`）——两路径一致（CPS 副作用
    /// 不重跑的对齐验证：若 CPS 重跑语句会重复打印，差分必然失败）。
    func testDiffJoinInsideCallArg() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffJoinInsideCallArg", filePath: #filePath) as String)
    }

    /// 意图：outer→inner（体内 `<=`）→leaf 调用链挂起——两路径一致（CPS 精确恢复、不重跑
    /// outer 已执行语句的对齐验证）。
    func testDiffCallChain() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffCallChain", filePath: #filePath) as String)
    }
    ...
    func testDiffJoinAll() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffJoinAll", filePath: #filePath) as String)
    }
    ...
    func testDiffIfBodyJoin() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffIfBodyJoin", filePath: #filePath) as String)
    }

    /// 意图：while 循环体内 `<=` 挂起（多轮迭代）——两路径一致（CPS 循环帧跨迭代持久、
    /// 条件重估/step 语义的对齐验证）。
    func testDiffWhileBodyJoin() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffWhileBodyJoin", filePath: #filePath) as String)
    }
    ...
    func testDiffForBodyJoinWithBreak() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffForBodyJoinWithBreak", filePath: #filePath) as String)
    }

    /// 意图：match 的 scrutinee 内 `<=`（`match (await child()):`）——两路径一致（CPS 在 match
    /// 值表达式内挂起的对齐验证）。
    func testDiffMatchJoinInValue() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffMatchJoinInValue", filePath: #filePath) as String)
    }

    /// 意图：match case 块体内 `<=` 挂起——两路径一致（CPS 在 caseEnv 内挂起/恢复的
    /// 对齐验证）。
    func testDiffMatchJoinInBody() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffMatchJoinInBody", filePath: #filePath) as String)
    }

    /// 意图：try 表达式内 `<=`（成功路径）——两路径一致（CPS 在 try 表达式内挂起的对齐验证）。
    func testDiffTryJoinInExpression() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffTryJoinInExpression", filePath: #filePath) as String)
    }

    /// 意图：try 表达式内 `<=`（错误路径，走 `except` 分支）——两路径一致（CPS 错误元组
    /// 提取与 except 分支的对齐验证）。
    func testDiffTryJoinInExpressionErrorPath() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffTryJoinInExpressionErrorPath", filePath: #filePath) as String)
    }
    ...
    func testDiffLabeledArgs() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffLabeledArgs", filePath: #filePath) as String)
    }

    /// 意图：`<=` join 后继续执行成员方法调用（`s.upper()`）——两路径一致（CPS 恢复后
    /// 成员分派语义的对齐验证）。
    func testDiffMemberMethodAfterJoin() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffMemberMethodAfterJoin", filePath: #filePath) as String)
    }

    /// 意图：函数体级 `defer` + 挂起 + return——两路径一致（CPS 下 defer 在 return 边界
    /// 执行、不影响返回值的对齐验证）。
    func testDiffDefer() throws {
        try assertSyncCPSMatch(try loadPiniFixture("testDiffDefer", filePath: #filePath) as String)
    }
}
