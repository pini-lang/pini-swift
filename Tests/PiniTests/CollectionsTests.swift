...
final class CollectionsTests: XCTestCase {
    ...
    func testArrayLiteralPrint() throws {
        let source = try loadPiniFixture("testArrayLiteralPrint", filePath: #filePath)
        XCTAssertEqual(try runProgram(source), "[1, 2, 3]\n")
    }

    // Intent: 验证空数组字面量 [] 打印为 "[]"
    func testEmptyArrayLiteralPrint() throws {
        let source = try loadPiniFixture("testEmptyArrayLiteralPrint", filePath: #filePath)
    ...
    func testNestedArrayLiteralPrint() throws {
        let source = try loadPiniFixture("testNestedArrayLiteralPrint", filePath: #filePath)
    ...
    func testDictionaryLiteralPrint() throws {
        let source = try loadPiniFixture("testDictionaryLiteralPrint", filePath: #filePath)
    ...
    func testSetLiteralPrintWithDedup() throws {
        let source = try loadPiniFixture("testSetLiteralPrintWithDedup", filePath: #filePath)
        // 保序去重：{3, 1, 2}
    ...
    func testCollectionsPassSemanticAndTypeCheck() throws {
        let source = try loadPiniFixture("testCollectionsPassSemanticAndTypeCheck", filePath: #filePath)
    ...
    func testArrayAppendFunctionalReturnsNewArray() throws {
        let source = try loadPiniFixture("testArrayAppendFunctionalReturnsNewArray", filePath: #filePath)
    ...
    func testArrayLastAndPopStackSemantics() throws {
        let source = try loadPiniFixture("testArrayLastAndPopStackSemantics", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testArrayLastPopOnEmptyReturnsNull() throws {
        let source = try loadPiniFixture("testArrayLastPopOnEmptyReturnsNull", filePath: #filePath)
        let output = try runProgram(source)
