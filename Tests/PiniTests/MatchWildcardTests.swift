...
final class MatchWildcardTests: XCTestCase {
    ...
    func testWildcardBlockExecutesOnNoCaseMatch() throws {
        let source = try loadPiniFixture("testWildcardBlockExecutesOnNoCaseMatch", filePath: #filePath)
    ...
    func testWildcardBlockSkippedOnCaseMatch() throws {
        let source = try loadPiniFixture("testWildcardBlockSkippedOnCaseMatch", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testPassAsWildcardBlockNoop() throws {
        let source = try loadPiniFixture("testPassAsWildcardBlockNoop", filePath: #filePath)
        let output = try runProgram(source)
    ...
    func testWildcardTakesOverDefault() throws {
        let source = try loadPiniFixture("testWildcardTakesOverDefault", filePath: #filePath)
        let output = try runProgram(source)
        XCTAssertEqual(output.trimmingCharacters(in: .whitespacesAndNewlines), "兜底\n结束",
